...
export function MultiSelectButtonsEditor(
  props: ISchemaContainerProps
): React.ReactElement {
  const { schema, path, value, errors, onFocus, onBlur } = props;
  ...
  function checks(isError: boolean) {
    ...
    if (!enums || schema["type"] !== "array") {
      throw `In schema ${JSON.stringify(
        schema
      )}, editor: checkButtons must be an array type with items an enum property`;
    }
