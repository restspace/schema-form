...
import { ErrorItem, validate } from "../error";
...
export function OneOfRadioEditor(
  ...
  useEffect(() => {
    if (context.outerPropsChange) {
      // only infer chosen option if the value change is a result of a props change to the external SchemaForm
      if (value !== null) {
        let newIdx = 0;
  ...
  const handleCheckChange = (idx: number) => () => {
    ...
    if (currentIdx >= 0) {
      dispatch(
        ValueAction.set(
          path,
          deleteSubschemaProperties(value, oneOf[currentIdx])
        )
      );
    }
  };
  ...
  function radios(isError: boolean, currentIdx: number) {
    ...
    const opts =
      ...
      oneOf.map(
        (subschema, i) =>
          (subschema["title"] as string) || "option " + i.toString()
      );
  ...
  const convErrors = (errors || []) as ErrorItem[];
