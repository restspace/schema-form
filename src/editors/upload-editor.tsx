...
export function sendFileAsBody(
  url: string,
  file: File,
  progress: (pc: number) => void,
  method: string = "POST"
  ...
  return new Promise<void>((resolve, reject) => {
    try {
      ...
      xhr.upload.onprogress = (ev: ProgressEvent) =>
        progress((ev.loaded / ev.total) * 100.0);
...
export function UploadEditor(props: ISchemaComponentProps) {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    ...
    if (schema["maximumSize"]) {
      const overMax = acceptedFiles.filter(
        (f) => f.size > schema["maximumSize"]
      );
      if (overMax.length > 0) {
        const maxKb = Math.floor(schema["maximumSize"] / 1000);
        const overMaxNames = overMax.map((f) => f.name).join(", ");
        const fileDesc =
          overMax.length > 1 ? "These files are" : "This file is";
        alert(
          `${fileDesc} over the maximum size of ${maxKb} KB: ${overMaxNames}`
        );
    ...
    if (schema["warningSize"]) {
      const overWarn = acceptedFiles.filter(
        (f) => f.size > schema["warningSize"]
      );
      if (overWarn.length > 0) {
        const warnKb = Math.floor(schema["warningSize"] / 1000);
        const overWarnNames = overWarn.map((f) => f.name).join(", ");
        const fileDesc =
          overWarn.length > 1 ? "These files are" : "This file is";
        if (
          !confirm(
            `${fileDesc} over the recommended size of ${warnKb} KB: ${overWarnNames}, do you want to upload them?`
          )
        ) {
          acceptedFiles = acceptedFiles.filter(
            (f) => f.size <= schema["warningSize"]
    ...
    if (schema["acceptedExtensions"]) {
      ...
      if (badExtensions.length > 0) {
        const fileDesc =
          badExtensions.length > 1 ? "These files have" : "This file has";
        ...
        acceptedFiles = acceptedFiles.filter((f) =>
          accepted.includes(getExt(f.name))
        );
      }
    }
    ...
    const sendFilePromises = acceptedFiles.map((file) => {
      ...
      return uploadContext
        .sendFile(absUrl, file, (pc) => updateProgress(file, pc))
        .then(() => encodeURI(absUrl));
    });
    Promise.all(sendFilePromises).then((absUrls) => {
      let saveUrls = absUrls.map((absUrl) =>
        uploadContext.saveSiteRelative ? makeSiteRelative(absUrl) : absUrl
      );
      if (isMulti) {
        saveUrls = [...urls, ...saveUrls];
  ...
  const onDelete = (absUrl: string) => () => {
    ...
    dispatch(
      ValueAction.set(
        path,
        urls
          .filter((v: string) => makeAbsolute(v, imageHost) !== absUrl)
          .join("|")
      )
    );
  };
  ...
  const images = (urls: string[]) => {
    const imageUrls = urls
      .filter(
        (url) =>
          imageSpec.extensions.indexOf(parseUrl(url).resourceExtension) >= 0
      )
      .map((url) => makeAbsolute(url, imageHost));
    const fileUrls = urls.filter(
      (url) => imageSpec.extensions.indexOf(parseUrl(url).resourceExtension) < 0
    );
    return (
      <div className="sf-image-container">
        ...
        {fileUrls.map((url) => (
          <div className="sf-upload-container" key={url}>
            ...
            <div
              className="sf-upload-delete"
              onMouseDown={onDelete(makeAbsolute(url, imageHost))}
            >
              x
            </div>
          </div>
