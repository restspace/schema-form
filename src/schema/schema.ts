...
export function fieldCaption(
  ...
  if (value) {
    let objectTitle =
      value && schema["objectTitle"] && value[schema["objectTitle"]];
  ...
  let idx: number | null =
    pathEl && pathEl.match(/^\d+$/) ? parseInt(pathEl) + 1 : null;
  return idx
    ? title
      ? title.replace("##", idx)
      : ""
    ...
    switch (prop) {
      ...
      case "propertyOrder":
        if (!schema["propertyOrder"]) {
          ...
          schema["propertyOrder"] = mergeOrders(
            schema["propertyOrder"],
            schema1["propertyOrder"]
          );
        }
        break;
      case "items":
      ...
      case "const":
        ...
        if (Array.isArray(schema[prop]) && schema[prop].length === 0) {
          return null;
        }
      ...
      case "allOf":
        break;
      default:
        if (schema[prop] && schema[prop] != schema1[prop]) return null;
        schema[prop] = schema1[prop];
        break;
    }
  }
    ...
    switch (prop) {
      ...
      case "required":
        if (!schema[prop]) schema[prop] = schema1[prop];
          ...
          schema[prop] = intersection<string>(
            <string[]>schema[prop],
            <string[]>schema1[prop]
          );
        break;
    ...
    for (let subSchema of <JSONSchema[]>schema["anyOf"]) {
      ...
      if (
        !context.validationErrors(
          nullOptionalsAllowed(schemaWithSubSchema),
          val
        )
      ) {
  ...
  (address: string) => {
    ...
    if (address.startsWith("#")) {
      ...
      if (idSegs[0]) {
        ...
        resolution = schemas.find(
          (s) => s["$id"] && s["$id"] === targetAddress
        );
      }
    }
  ...
  if (schemaType === "object" && value && typeof value === "object") {
    if (schema["properties"]) {
      Object.keys(schema["properties"]).forEach((prop) => {
        const res = deleteSubschemaProperties(
          value[prop],
    ...
    return (value as any[])
      .map((item) => deleteSubschemaProperties(item, schema["items"] || {}))
